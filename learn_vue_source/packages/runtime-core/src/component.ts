...
function setupStatefulComponent(
  ...
  if (setup) {
    // setup有参数, 那么会创建一个setupContext上下文, 这个上下文可以在函数中被使用, 比如 {emit, slots} 等
    ...
    pauseTracking()
    // 执行setup函数, 并且获取结果
    // 内部就是执行了一下setup函数, 并且把instance.props, setupContext作为参数进去了
    ...
    if (isPromise(setupResult)) {
      ...
      // 处理setup的结果
      // 内部进行判断, 如果是一个函数, 那么就是render函数
      // 如果是一个对象, 那么就放到setupState中
...
function finishComponentSetup(
  ...
  if (__NODE_JS__ && isSSR) {
    ...
    // 组件的render函数 赋值给instance的render函数
  ...
  if (__DEV__ && !Component.render && instance.render === NOOP && !isSSR) {
    ...
    if (!compile && Component.template) {
      // 如果编写了template, 但是使用的是runtime-only版本
      warn(
      ...
      // 既没有写template, 也没有写render function
